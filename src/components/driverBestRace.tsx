import { DriverResults, Race } from "@/util/fetchDriverRaceResults";
...
export default function DriverBestRace({ driverResults }: Props) {
  const bestResult = driverResults?.races.reduce(
    (previousValue: string, race) => {
      const position = race.results?.[0]?.position || "22";
  ...
  const worstResult = driverResults?.races.reduce(
    (previousValue: string, race) => {
  ...
  const lastRaceResult =
    driverResults?.races[driverResults?.races?.length - 1].results[0].position;
      ...
      <div className="bg-white px-4 py-2 rounded-lg dark:bg-dark transition-colors duration-700">
        ...
        <div className="text-sm font-semibold text-gray-500 dark:text-gray-400 pt-2 flex items-center pb-2">
          ...
          <p className="bg-[#fffbc3] dark:bg-[#fff34d6c] dark:text-yellow-400 text-yellow-600 px-4 py-1.5 rounded font-semibold ml-auto w-[65px] text-center">
            {calculateAverageRaceResult(driverResults?.races)}
          </p>
        </div>
      </div>
    </div>
...
function calculateAverageRaceResult(arr: Race[] | undefined) {
  ...
  const sum = arr.reduce(
    (acc, currentRace) => acc + Number(currentRace.results[0].position),
    0
  );
